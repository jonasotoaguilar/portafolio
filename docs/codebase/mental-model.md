...
- **Five views** — each route is its own static HTML page with a per-view title, exactly one `h1` inside a single `<main>`, JSON-LD, and a sitemap entry. PROJECTS uses an in-view LIST + detail panel (deep links use fragments, `/projects#serviceflow`); RESUME renders a dark Persona scene (upper-left title polygon with the degree, description on the field, compact experience list with selectable descriptions — focus stays in the list —, a dominant central figure, a right collage, and a wide navy summary trapezoid); SKILLS renders a ranked diagonal card list (name + category + RANK 1..4, red geometric active layer).
...
- **Resume content** — a `resume` content collection typed from Jona's August 2026 CV, English-only (degree, experience with descriptions, professional summary), schema-validated, with a no-phone privacy gate; no ranks, levels, or metrics, and no projects/skills/languages sections (they have dedicated views).
