...
- **Five views** — each route is its own static HTML page with a per-view title, exactly one `h1` inside a single `<main>`, JSON-LD, and a sitemap entry. PROJECTS and RESUME use an in-view LIST + detail panel (PROJECTS deep links use fragments, `/projects#serviceflow`); SKILLS renders a ranked diagonal card list (name + category + RANK 1..4, red geometric active layer).
