...
- **Root shell** — `/` is a full-screen game menu of exactly five items (ABOUT, RESUME, PROJECTS, SKILLS, CONTACT), each a real route link; keyboard-first (ArrowUp/ArrowDown wrap, Enter activates) with key hints only — no Gamepad API.
...
- **Test migration** — shell/list-detail tests replace the anchor/scroll/section tests; E2E `views.spec.ts` covers routes, zero-JS, keys, 404, view-transition overlays, and the full-page fallback.
...
Astro Pages & Layouts (src/pages/, src/layouts/)   [shell + five view routes]
        |
        v
Astro/Vite build  +  Tailwind CSS 4  +  client scripts (shell/list-detail keys, canvas)
        |
        v
Static output (HTML/CSS/JS) -> any static host / CDN
        |
        v
Browser: View Transitions + CSS layers + Canvas layer (progressive enhancement)
```
...
2. `src/styles/global.css` — Tailwind entry point (tokens, glow/scanlines, view-transition overlays, reduced motion).
3. `src/components/game/*` + `src/scripts/{shell,view}.ts` — shell menu and view LIST/detail behavior.
4. `astro.config.mjs` — build configuration (site URL, sitemap, Tailwind plugin).
5. `PRD.md` and `DESIGN.md` — the product and visual system.
6. `ARCHITECTURE.md` and `docs/adr/` — how the site is built and why.
7. `openspec/changes/persona-game-menu-navigation/` — the delivered change: proposal, design, six delta specs, tasks.
