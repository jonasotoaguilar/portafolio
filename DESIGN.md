...
- `Head` + `Footer`: per-page title/description/canonical/OG/JSON-LD (`Person`, no `telephone`, no unverified `linkedin.com`, `sameAs` is GitHub only, `url` falls back to GitHub when site absent); footer repeats non-affiliation and MIT line.
