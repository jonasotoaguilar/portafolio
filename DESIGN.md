...
- **LIST/detail**: a LIST column of `list-item` surfaces used by PROJECTS; the selected item (`list-item-active`) opens a `detail-panel` on the right with badges, description, stack, and external link. Selection state is per-view client-side; PROJECTS deep links (`#slug`) preselect an item. Focus moves into the panel on open and back to the list on close. SKILLS is card-only and renders no detail panel.
- **RESUME (dark Persona scene)**: a full-bleed cyan/electric-blue/deep-navy scene (the one view that does not share the light field). Route-local layers: a dominant central portrait figure (registered `persona_1` derivative, ~40vw × ~90vh, behind the left rail and the summary plate), a right-side collage of sprite derivatives (clock, computer, night city — staggered, cropped at the right edge), an upper-left primary polygon born outside the left viewport edge (black upper mass with the white `RESUME` title, white lower mass with the degree title/meta, strong diagonal right edge via `clip-path`), the description region directly on the blue field (no card), a compact experience list (label above, rows 32–44px; selected row white with dark text and a thin cyan accent; unselected rows transparent deep-blue with cyan text and separators), and a lower-right **wide horizontal navy trapezoid** summary plate whose `PROFESSIONAL` title escapes the top edge, supported by a lighter cyan triangle, with `SUMMARY` immediately to its right and compact white body copy. Selection is per-view client-side with no deep links: ArrowUp/ArrowDown (wrapping), Enter, ArrowRight, Space, and click select the entry and swap the description region in place — keyboard focus never leaves the list. Without JS every entry's description renders in document flow below the degree. Mobile stacks the same polygon/cyan/navy language in a single coherent column.
...
| `/resume` | Dark Persona scene: title polygon, selectable experience descriptions, summary trapezoid | Menu item RESUME | Select an experience; Esc to menu |
...
- RESUME content is English-only and derives only from the verified CV: the USACH degree "Computer Science and Informatics Engineer" (Mar 2020-Apr 2025), the experience entries (Productos Barber Chile 2020-2026; Policomp IT support internship Jan-Mar 2020) with professional descriptions, and the professional summary. Projects and skills do not appear (they have dedicated views). No ranks, metrics, or phone number.
...
- No icon libraries, no animation libraries beyond `motion`, no CSS framework beyond Tailwind. The only image assets ship on the RESUME view as re-encoded derivatives of the registered owner-created artwork (`assets/persona` figure + `assets/sprites` collage), never original bytes.
