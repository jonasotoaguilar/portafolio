...
| `/resume` | `resume.astro` | Dark scene: title polygon, selectable EXPERIENCE descriptions, PROFESSIONAL SUMMARY trapezoid |
| `/projects` | `projects.astro` | LIST of the four verified projects + detail panel; `#slug` deep links |
| `/skills` | `skills.astro` | Ranked skills: fixed seven-slot recycled list (name + category + RANK 1..4), diagonal stagger, red geometric active layer; explicit editable rank data in `skills.yaml`; the enhanced controller (`skills-scroll.ts`) recycles seven persistent slot nodes from a 22-record inline data blob — no native scrolling |
...
- **Responsibility**: Single source of truth for the four projects, grouped ranked skills, and site config (name, role, tagline, focus areas, email, socials). The PROJECTS view renders exactly the four verified entries (`assertExactlyFour`, `sortByOrder`); SKILLS renders explicit editable rank entries — the schema validates `{ name, rank }` objects with `rank` on the 1..4 scale (1 basic, 2 intermediate, 3 advanced, 4 expert) and rejects plain strings, out-of-scale/non-integer ranks, and unknown keys. Bounded, file-based content; no runtime dependencies; missing or mistyped front matter fails the build with a schema error.
- **Resume content source**: the RESUME view renders only verified CV data (August 2026 CV), English-only: the USACH degree "Computer Science and Informatics Engineer" (Mar 2020–Apr 2025); the Productos Barber Chile experience (2020–2026) and the Policomp IT support internship (Jan–Mar 2020), each with a professional description; and the professional summary. Projects and skills do not appear (they have dedicated views). No ranks, metrics, or phone number are published. The typed `resume` collection (`resume.yaml` in `src/content/`) schema-validates degree, experience, and summary like the other collections, with a no-phone privacy gate enforced by `scripts/verify-no-phone.mjs`.
...
- **Technology**: `.glow-layer` in `global.css`, scoped to `html[data-route]:not([data-route="404"])` — the static white→light-blue→sea-blue diagonal gradient replaces the breathing navy radial glow on the shell and all views except RESUME; the 404 route keeps the dark glow as its error identity, and the RESUME view covers the field with its own opaque dark scene (route-local layers in `resume.astro`). Light-field palette tokens (`surface-light`, `surface-light-muted`, `text-on-light`, `text-on-light-secondary`, `accent-on-light`) map to Tailwind utilities; semantic surface classes (`.view-list-item`, `.view-panel`, `.view-chip`, `.view-link`) live in `global.css` and are consumed by the shared view components and pages.
- **Responsibility**: one product-wide visual language, with the RESUME view as the documented dark-scene exception (it sets its own text colors explicitly). Body copy on light-field routes defaults to `text-on-light`; view headings, panels, list items, chips, links, and the control cluster (key hints + mute toggle switch to white text with a subtle black outline, same as the shell) all render against the light field. The selected list item echoes the shell menu: muted-light surface, `accent-on-light` text, and a 2px cyan bottom bar (the CSS keys on the presence of `data-active` because `view.ts` toggles the attribute to an empty value after load).
...
- **Responsibility**: PROJECTS renders a LIST column of `list-item` surfaces; the selected item opens a `detail-panel` with badges, description, stack, and external links. SKILLS is card-only (see the skills rank-card list below) and renders no detail panel. RESUME is not a detail-panel view: its route-local `src/scripts/resume.ts` drives a compact experience list whose selection swaps a single reserved description region in place, with focus staying on the list.
- **State boundaries**: selection lives only inside the view's script; the URL carries at most a fragment (`/projects#serviceflow`), which preselects on load. Opening a panel moves focus into it; closing returns focus to the list item. On mobile the panel stacks below the list and gains its own internal scroll region (fixed list size: 4 projects). RESUME selection is also client-side: arrows/Enter/Space/click update the description region while focus stays on the compact list (2 experience entries), and no deep links exist.
- **Failure modes**: Without JS all detail content is rendered in document flow; deep-link fragments scroll to the entry.

### Skills rank-card list

- **Technology**: `SkillListItem.astro` (one fallback card per skill) rendered inside the shared `GameList`; CSS in `global.css` under the skills contract: a per-card `--skill-index` custom property drives the diagonal stagger and REPEATS by slot (`index % 7`), `skewX(-8deg)` leans cards with counter-skewed content, and the active card's red parallelogram is a `::before` layer (`accent-red`, translated + skewed, `pointer-events-none`) keyed on the `data-active` presence state — geometry, never a `box-shadow`. The enhanced view is a FIXED-SLOT RECYCLED LIST, not a native scrollable 22-card list: the skills page renders all 22 fallback cards in normal document flow plus an inline `type="application/json"` data blob; on successful init, `src/scripts/skills-scroll.ts` (imported only by `/skills`) adopts the first seven cards as persistent slot buttons (`data-skill-slot`), removes the remaining fifteen, and gates the enhanced CSS (`data-skills-enhanced` on the region → `flex: 1; min-height: 0; max-height: var(--skills-window-height)` on the region and `overflow: hidden` on `.skills-viewport`, so the stage is exactly seven row pitches and nothing ever scrolls). The slot nodes are never replaced — only their text/category/rank and ARIA attributes mutate (`role="listbox"` on the list; `role="option"` + `aria-selected`, `aria-setsize="22"`, `aria-posinset`, roving `tabIndex` on the slots; `data-list-item` is dropped so the generic `view.ts` no-ops on Skills while its document-level Escape keeps working). Pure state transitions live in `src/lib/skills/window.ts` (`{activeIndex, windowStart, focusedSlot}` plus wheel threshold/accumulation math); `src/lib/skills/scroll.ts` is deleted. The diagonal scrollbar near the list's lower-left is now a fixed decorative affordance: static thumb (no `scrollTop` to mirror), `pointer-events: none` on track and thumb, never a drag control, `aria-hidden`, hidden on coarse pointers. Click/hover effect sounds run through the shared delegated wiring in `src/lib/audio/effects.ts`, which now also targets `[data-skill-slot]` — exactly one click/select sound per interaction, never duplicated by the controller.
- **Responsibility**: Render every skill exactly once — in the no-JS fallback as all 22 flattened, ordered cards (name + category derived from the group key: Backend/Frontend/Tooling on the left, `RANK` + numeric value on the right), and in the enhanced view as the seven persistent slots whose content recycles through the 22 records as the data window advances (initial window 1..7, focus on slot 1). Ranks are explicit editable content from `skills.yaml` (1 basic, 2 intermediate, 3 advanced, 4 expert) — the schema rejects anything else. Interaction: ArrowDown/ArrowUp move focus through the slots without moving the window; at slot 7/slot 1 the next press advances/moves-back the data window exactly one skill with focus visually stuck to that edge (last→first wraps to window 1–7/slot 1, first→last wraps to window 16–22/slot 7); wheel deltas accumulate (≈53px threshold) and emit the same discrete one-step transitions — `scrollTop` stays 0, Ctrl+wheel is never hijacked; clicking a slot selects it in place (window unchanged). Enter keeps the list primary (no panel), Escape returns directly to `/`. The slot's accessible name composes skill + category + rank from visible text.
- **State boundaries**: the enhanced skills state (`activeIndex`/`windowStart`/`focusedSlot`) is owned solely by `skills-scroll.ts`; no panels, no `#slug` deep links. `view.ts` keeps its generic list behavior for Projects unchanged (Resume now runs its own `src/scripts/resume.ts`) and only skips ArrowUp/Down inside the skills viewport (its `skillsOwnsArrows` guard); with no `data-list-item` in the enhanced DOM it also finds nothing to wire on Skills.
- **Failure modes**: Without JS (or when init fails — missing data blob, fewer than seven cards) the 22-card fallback stays in normal document flow (stagger and red layer are pure CSS, SSR `data-active="true"` on the first card) and the page scrolls; coarse/mobile layouts collapse the stagger to full-width ≥44px targets; short screens shrink the window with the available space instead of overflowing the page.
...
- **Responsibility**: Shell: ArrowUp/ArrowDown/Enter. Views: ArrowUp/Down move the LIST, Enter/ArrowRight open detail, Escape closes one level (panel) and then returns to `/`; SKILLS is card-only — ArrowUp/Down run the fixed seven-slot recycled list (owned by `skills-scroll.ts`, which `view.ts` delegates to for arrow keys inside the skills viewport): slot moves first, then one-skill window shifts with edge-stuck focus, then wraps; wheel emits the same discrete transitions; Enter keeps the list primary (no panel exists), and Escape returns directly to `/`; Browser Back follows native history. One scoped handler per active screen; the existing "no hijack when closed" test pattern extends to views; unsupported keys no-op.
...
| E2E | `@playwright/test` | 1.62.1 | Shell menu navigation (active indicator `aria-current`/`data-active`, stagger non-overlap, coarse-pointer collapse), four view routes, LIST/detail + `#slug` deep links, ranked SKILLS recycled list (exactly seven persistent `[data-skill-slot]` options with fixed geometry, listbox semantics, posinset/setsize, diagonal stagger collapse on mobile, red active parallelogram, slot clicks selecting in place), skills keyboard/wheel contract (six slot moves then one-skill window shifts with edge-stuck focus, both wraps, wheel one-step threshold with accumulation, `scrollTop`/`window.scrollY` staying 0, Ctrl+wheel untouched, exactly one select sound per transition, click/hover effects once per slot), skills fixed diagonal scrollbar (static thumb, no drag, no pointer targets, lower-left placement, no horizontal overflow), no-JS fallback (all 22 cards in flow), Escape/Back return, no-keyboard-hijack, control cluster placement, ambient audio (no-track state, gesture gate, mute persistence, navigation survival), mobile stacked detail, 404 page, reduced-motion static canvas + removed-layer absence, link checks, sitemap presence, global light-field gradient on shell/views vs dark 404 |
...
| In-view LIST/detail panels with `#slug` deep links over detail sub-routes | Matches the game-panel interaction; per-project pages stay a v2.0 option | `/projects/:slug` sub-route — see ADR-0003 |
...
| Skills list taller than the viewport | Cards below the fold unreachable on desktop | The skills view is a fixed-slot recycled list: seven persistent slots with fixed geometry, no native scrolling (`overflow: hidden`, `scrollTop` always 0) — all 22 records stay reachable through discrete wheel/keyboard window transitions (`skills-scroll.ts` + `src/lib/skills/window.ts`); a fixed diagonal scrollbar affordance marks the lower-left; coarse/mobile layouts collapse the stagger to full-width targets and the no-JS fallback keeps all 22 cards in document flow |
