...
- **Responsibility**: PROJECTS and RESUME render a LIST column of `list-item` surfaces; the selected item opens a `detail-panel` with badges, description, stack, and external links. SKILLS is card-only (see the skills rank-card list below) and renders no detail panel.
- **State boundaries**: selection lives only inside the view's script; the URL carries at most a fragment (`/projects#serviceflow`), which preselects on load. Opening a panel moves focus into it; closing returns focus to the list item. On mobile the panel stacks below the list and gains its own internal scroll region (fixed list sizes: 4 projects, 5 resume sections).
...
- **State boundaries**: the enhanced skills state (`activeIndex`/`windowStart`/`focusedSlot`) is owned solely by `skills-scroll.ts`; no panels, no `#slug` deep links. `view.ts` keeps its generic list behavior for Projects/Resume unchanged and only skips ArrowUp/Down inside the skills viewport (its `skillsOwnsArrows` guard); with no `data-list-item` in the enhanced DOM it also finds nothing to wire on Skills.
